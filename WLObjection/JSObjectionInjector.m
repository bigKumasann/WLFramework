...
//@property (nonatomic, strong) NSMutableArray *snModules;
...
static NSMutableArray *_snModules = nil;
...
- (instancetype)initWithContext:(NSDictionary *)theGlobalContext {
    if ((self = [super init])) {
        ...
        if (!_snModules) {
            _snModules = [[NSMutableArray alloc] init];
        }
...
#pragma mark - 苏宁扩展
//收集module对象,然后统一configure,否则configure重复调用。by chupeng 2016-1-28
- (void)collectModule:(JSObjectionModule *)module
{
    if (![_snModules containsObject:module])
    {
        [_snModules addObject:module];
    }
}

- (id)registerAndConfigureModules
{
    return [self withModuleCollection:_snModules];
}

- (void)afterConfigureModules
{
    for (JSObjectionModule *module in _modules) {
        if ([module respondsToSelector:@selector(didConfigured)])
        {
            [module didConfigured];
        }
    }
